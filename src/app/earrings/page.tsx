...
const EarringCard = ({ earring, index }: { earring: typeof placeholderEarrings[0], index: number }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        <p className="text-sm text-gray-300 mt-1 min-h-[3em]">{earring.description}</p>
      </div>
    </div>
  );
};
      ...
      <div className="space-y-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
          ...
          {filteredEarrings.slice(0, visibleCount).map((earring, index) => (
            <EarringCard key={earring.id} earring={earring} index={index} />
          ))}
        </div>
